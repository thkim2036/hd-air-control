...
function initTable() {
  table = new Tabulator(gridContainer.value, {
    ...
    rowFormatter: function (row) {
      ...
      // 공장(blaCell) 값이 다르면 병합하지 않음
      if (rowData.blaCell !== prevData.blaCell) return;

      const mergeFields = ["blaCell", "shipNoList", "blockList", "workList",
      ...
      mergeFields.forEach((field) => {
        ...
        if (rowData[field] === prevData[field]) {
          // 동일한 값이면 숨김 + border 제거 → 마치 merge된 것처럼 보임
          cell.getElement().style.color = "transparent";
...
async function loadData() {
  ...
  try {
    ...
    const data = raw.map(item => {
      ...
      console.log('isTargetFactory... ', isTargetFactory)
